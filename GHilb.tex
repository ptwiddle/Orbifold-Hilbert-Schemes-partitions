...
\begin{definition}
Let $v\in S^0_G$.  Then let $\tilde{v}\in K^0(G)$ be the smallest class so that $\Hilb_{\core(v)}(\C^2/G)$ is nonempty.
\end{definition}



\begin{lemma} Suppose that $G\in SL_2$. Then
$$\tilde(v)=v+(\frac{1}{2}Q_M(v,v)-v_0)[V_G]$$
In particular, we see $\tilde(v)$ is a quadratic function of $v$.
\end{lemma}

\begin{proof}
By definition, $\tilde(v)=v+M[V_G]$ for some $M\in(1/|G|)\Z]$; we must find $M$.

Let $\OO_{\tilde{v}}=R/\mathcal{I}$ for some element $\mathcal{I}\in\Hilb_{\tilde{v}}$.  

Then 
$$\chi(\tilde{v}, \tilde{v})=\Ext^0(\OO_{\tilde{v}},\OO_{\tilde{v}})-\Ext^1(\OO_{\tilde{v}},\OO_{\tilde{v}})+\Ext^2(\OO_{\tilde{v}},\OO_{\tilde{v}})$$
Since $G\in SL_2$, and $\tilde{v}$ is the class of a core, we have $\dim\Hilb_{\tilde{v}}=0$, and hence $\Ext^1(\OO_{\tilde{v}},\OO_{\tilde{v}})=0$.

Since $\OO_{\tilde{v}}$ is a quotient of $R$, which is generated by a single element of trivial weight, $\Ext^0(\OO_v,\OO_v)=\tilde{v}_0=v_0+M$.

Since $G\subset SL_2$, we have $K_{[C^2/G]}=\OO_{[\C^2/G]}$, and so Serre duality gives that $\Ext^2(\OO_{\tilde{v}},\OO_{\tilde{v}})=v_0+M$ as well.  Thus, we have found
$$\chi(\tilde{v},\tilde{v})=2v_0+2M$$
On the other hand, Riemann-Roch and the fact that $V_G\in\Ker(Q_{M})$ gives 
$$\chi(\tilde{v},\tilde{v})=\chi(v+M[V_G],v+M[V_G])=Q_M(v+M[V_G],v+M[V_G])=Q_M(v,v)$$
Thus $M=\frac{1}{2}Q_M(v,v)-v_0$.
\end{proof}
...
We have
$$M_v=\frac{1}{2}\left(Q_M(v)+\dim\Hilb_{\tilde{v}} -v_0-v_{k}\right)$$
...
\begin{proof}
As before, we calculate $\chi(\tilde{v},\tilde{v})$ in two different ways.

Letting $\OO_{\tilde{v}}$ be any quotient sheaf with the right $K$-theory class, we first use the definition.

We have $\Ext^0(\tilde{v},\tilde{v})=\tilde{v}_0=v_0+M$, as before.  Now $\Ext^1$ is no longer zero, but instead equal to $\dim \Hilb_{\tilde{v}}$.  Finally, by Serre duality 
$$\Ext^2(\OO_{\tilde{v}}, \OO_{\tilde{v}})=\Ext^0(\OO_{\tilde{v}}, \OO_{\tilde{v}}\otimes K)$$ 
Again, $\OO_{\tilde{v}}$ being a quotient of $R$ is generated by a single $G$ invariant element, which can be mapped to any $G$-invariant element of $\OO_{\tilde{v}}\otimes K$, which are the elements that act as the opposite of $k$ in $\OO_{tilde{v}}$.  

Recalling Warning \ref{warning-} that $G$ acts on monomials with opposite action to the tangent bundle, we actually want a monomial that transf


 and so $\Ext^2(\OO_{\tilde{v}}, \OO_{\tilde{v}})=M+v_{-k}$.  Putting all this together, we see that
$$\chi(\tilde{v},\tilde{v})=2M+v_0+v_{-k}-\dim\Hilb_{\tilde{v}}$$
Calculating the Euler pairing using Riemann-Roch works exactly the same way, and we have
$$\chi(\tilde{v},\tilde{v})=Q_M(v,v)$$
...
\begin{proof}
Let $\Delta$ be a large triangular partition, with all horizontal steps being the same color.  It is obvious that we can take $\Delta$ large enough to contain both $\lambda$ and its $G$-core $\tilde{\lambda}^G$.  We claim that $\Delta\setminus\lambda$ and $\Delta\setminus\tilde{\lambda}^G$ have the same class in $\overline{K}(G)$.  
